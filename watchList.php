    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-12 text-white">
                <label class="form-label fs-1" for="">WatchList</label> <i class="bi bi-heart-fill fs-4"></i>
            </div>
            <div class="col-12 ">
                <div class="row">
                    <div class="col-12">
                        <div class="row">
                            ...
                            $page_data = DATABASE::search("SELECT * FROM `watchlist` WHERE `user_email`= '" . $user . "'");
                            ...
                            $shipping = "0";
                            $items="0";
                            $cart_data = DATABASE::search("SELECT * FROM `watchlist` WHERE `user_email`='" . $user . "'  LIMIT " . $results_per_page . " OFFSET " . $page_first_result . " ");
                            ...
                            ?>
                                ...
                                for ($x = 0; $x < $cart_data_row; $x++) {
                                    ...
                                    $product_data = DATABASE::search("SELECT `product`.`title`,`watchlist`.`qty`,`color`.`color`,`product_images`.`image`,`product`.`price`,`product`.`delivery_fee` FROM `product` INNER JOIN `color`  ON `product`.`color_id` =`color`.`id` INNER JOIN `product_images` ON `product`.`id`=`product_images`.`product_id` INNER JOIN `watchlist` ON `product`.`id`=`watchlist`.`product_id` WHERE `product`.`id`='" . $product_id . "'  LIMIT 1 ;");
                                                            ...
                                                            <div class="row">
                                                                <div class="col-12">
                                                                    <a class="btn btn-outline-primary d-grid col-10 offset-1" href="<?php echo "singleproductview.php?id=".($product_id) ?>">Buy
                                                                        Now</a>
                                                                    <button class="btn btn-outline-danger d-grid col-10 offset-1 mt-2" onclick="deletefromWatchList(<?php echo $cart_id; ?>);">Remove
